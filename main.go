...
func Where() {
	...
	return
	//1、通过字段查询 eq
	sql, err := table1.GetDbContext(context.Background()).Where(&table1.Name, orm.Eq, "a").ToFirstOrDefaultSql()
	if err != nil {
		panic(err)
	}
	if sql != "SELECT * FROM `Table1` WHERE `name` = 'a' AND `is_deleted` = 0 ORDER BY `Table1`.`id` LIMIT 1" {
		panic("where")
	}
	//FirstOrDefault：SELECT * FROM `Table1` WHERE `name` = 'a' AND `is_deleted` = 0 LIMIT 1

	//2、通过字段查询 表别名
	sql, err = table1.GetDbContext(context.Background()).SetTable("t").Where(&table1.Name, orm.Eq, "a", "t").ToFirstOrDefaultSql()
	if err != nil {
		panic(err)
	}
	if sql != "SELECT * FROM `Table1` as t WHERE `t`.`name` = 'a' AND `t`.`is_deleted` = 0 ORDER BY `t`.`id` LIMIT 1" {
		panic("where")
	}
	//FirstOrDefault：SELECT * FROM `Table1` as t WHERE `t`.`name` = 'a' AND `t`.`is_deleted` = 0 LIMIT 1

	//3、通过字段查询 StartWith
	sql, err = table1.GetDbContext(context.Background()).Where(&table1.Name, orm.StartWith, "a").ToFirstOrDefaultSql()
	if err != nil {
		panic(err)
	}
	if sql != "SELECT * FROM `Table1` WHERE `name` Like 'a%' AND `is_deleted` = 0 ORDER BY `Table1`.`id` LIMIT 1" {
		panic("where")
	}
	//FirstOrDefault：SELECT * FROM `Table1` WHERE `name` Like 'a%' AND `is_deleted` = 0 LIMIT 1

	//4、通过字段查询 IsNotNull
	sql, err = table1.GetDbContext(context.Background()).Where(&table1.Name, orm.IsNotNull, "").ToFirstOrDefaultSql()
	if err != nil {
		panic(err)
	}
	if sql != "SELECT * FROM `Table1` WHERE `name` IS NOT NULL  AND `is_deleted` = 0 ORDER BY `Table1`.`id` LIMIT 1" {
		panic("where")
	}
	//FirstOrDefault：SELECT * FROM `Table1` WHERE `name` IS NOT NULL  AND `is_deleted` = 0 LIMIT 1
...
func Select() {
	...
	fmt.Println(errr)
	sql, err := table1.GetDbContext(context.Background()).SetTable("a").Select(&table1.Id).ToFirstOrDefaultSql()
	if err != nil {
		panic(err)
	}
	fmt.Println(sql)
